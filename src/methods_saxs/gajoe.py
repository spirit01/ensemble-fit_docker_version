...
def make_experiment(all_files, tmpdir, verbose, verbose_logfile, path, mydirvariable):
    ...
    #  Curve no.     1
    # 0.309279E+08
    num_lines = sum(1 for line in open(f'{mydirvariable}{all_files[0]}.dat')) - 2
    with open(f'{tmpdir}/method/juneom.eom', 'w') as file1:
        file1.write(f'    S values   {num_lines} \n')
        with open(f'{mydirvariable}{all_files[0]}.dat') as file2:
            for line in file2:
                if line.startswith('#'):
                    continue
        ...
        for i, filename in enumerate(all_files, start=1):
            with open(f'{mydirvariable}{filename}.dat') as file2:
...
def collect_results(tmpdir, all_files):
    ...
    structure_weight = []
    m = re.compile('^\s*\d+\)')
    with open(f'{tmpdir}/results/GA001/curve_1/logFile_001_1.log') as file_gajoe:
        for line in file_gajoe:
            ...
            if p:
                ...
                structure_weight.append((f'{all_files[index]}.pdb', weight))
